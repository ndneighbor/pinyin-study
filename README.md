...
4. Refresh any webpages already open.
5. Select a Chinese character or phrase on a webpage to see pinyin. You can also open Pinyin Study from Chrome’s puzzle-piece Extensions menu and type or paste text.
...
- Select up to 300 characters. Press Escape, scroll, or click elsewhere to close the card.
- Runs on normal HTTP/HTTPS webpages. Chrome settings, the Chrome Web Store, built-in PDF viewer, images, and embedded frames are not supported. Use the popup to paste text from these places.
- Selection lookup skips editable fields. Use the popup for text you are writing.
- Everything is processed locally. No network requests, tracking, accounts, or saved browsing history. Chrome may show a website-access warning because the extension detects text selections on webpages.
